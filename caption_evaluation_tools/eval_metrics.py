...
def evaluate_metrics_from_files(
    ...
    cocoEval = COCOEvalCap(coco, cocoRes, predictions)
...
def evaluate_metrics_from_lists(
    ...
    rand_id = uuid.uuid1()
    ref_file = tmp_dir.joinpath(f"ref_{rand_id}.json")
    pred_file = tmp_dir.joinpath(f"pred_{rand_id}.json")

    write_json(ref, ref_file)
    write_json(pred, pred_file)

    metrics, per_file_metrics = evaluate_metrics_from_files(
        pred_file, ref_file, predictions
    )
...
def check_and_read_csv(
    path: Union[str, Path, List[Dict[str, str]]]
) -> List[Dict[str, str]]:
...
def combine_single_and_per_file_metrics(
...
def evaluate_metrics(
    prediction_file: Union[str, Path, List[Dict[str, str]]],
